...
static void prefill_sequential(unsigned long tot_nelems, uint64_t *field, uint64_t elt_size)
{
  uint64_t i;
  uint64_t index1;
  char data[elt_size];
...
static void *do_gups(void *arguments)
{
  ...
  int access_type = args->access_type;
  uint64_t ret = 0;
...
int main(int argc, char **argv)
{
  ...
  if (argc != 8) {
    fprintf(stderr, "Usage: %s [threads] [updates per thread] [exponent] [data size (bytes)] [hot set exponent] [hot set access proportion] [access type]\n", argv[0]);
  ...
  switch (access_type) {
    case 0://read and update (original)
      fprintf(stderr, "Access type: read and write\n");
      break;
    case 1://read only
      fprintf(stderr, "Access type: read only\n");
      break;
    case 2://write only
      fprintf(stderr, "Access type: write only\n");
  }
